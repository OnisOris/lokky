...
from lokky.pionmath import (
    SSolver,
)  # import the SSolver class from the lokky module

# Number of points
n_points = 3

# Set parameters for SSolver
params = {
...
np.random.seed(42)
positions = np.random.rand(n_points, 3) * np.array(
    [10, 10, 0]
)  # points in the range [0,10] on all axes
...
# Create a window with 4 subplots (2x2): Top, Side, Front, and 3D views
...
# Disable 3D navigation to avoid conflicts
...
# Create scatter plots for all projections
...
def draw_arrows_3d():
    """Redraw arrows in the 3D projection using quiver."""
    global error_quivers_3d, ctrl_quivers_3d
...
def update_scatter():
    """Update scatter plot data for all projections."""
    ...
    # For 3D scatter, update the data manually:
...
# Global variables for the selected point and active projection view
selected_index = None
selected_view = None  # can be: "top", "side", "front", "3d"
last_y = None  # for vertical offsets in 3D


# Helper functions for the 3D projection
...
def on_press(event):
    """On press, find the nearest point in the active projection view."""
    global selected_index, selected_view, last_y
    ...
    if event.inaxes == ax_top:
        for pos in positions:
            distances.append(np.hypot(pos[0] - click[0], pos[1] - click[1]))
        current_view = "top"
    elif event.inaxes == ax_side:
        for pos in positions:
            distances.append(np.hypot(pos[0] - click[0], pos[2] - click[1]))
        current_view = "side"
    elif event.inaxes == ax_front:
        for pos in positions:
            distances.append(np.hypot(pos[1] - click[0], pos[2] - click[1]))
        current_view = "front"
    elif event.inaxes == ax_3d:
        # For 3D, use point projection
        click_screen = np.array([event.x, event.y])
        for pos in positions:
            proj = get_2d_coords(ax_3d, pos[0], pos[1], pos[2])
            distances.append(
                np.hypot(proj[0] - click_screen[0], proj[1] - click_screen[1])
            )
        current_view = "3d"
    distances = np.array(distances)
    threshold = (
        15 if current_view == "3d" else 0.5
...
def on_motion(event):
    """On mouse motion, update the coordinates of the selected point based on the active projection view."""
    global selected_index, positions, state_matrix, control_signals, last_y
    ...
    if event.inaxes == ax_top and selected_view == "top":
        ...
        # In 3D projection, update x and y via projection, and z with modifiers (Shift/Ctrl)
        if event.key is not None and (
            ...
            if last_y is not None and event.y is not None:
                ...
                state_matrix[selected_index, 2] = positions[selected_index, 2]
        else:
    ...
    # Recalculate control signals and update the plots
    control_signals = solver.solve(state_matrix, target_matrix, dt)
    update_scatter()
    draw_arrows_2d()
    draw_arrows_3d()
...
def on_release(event):
    """On release, clear the selected point and active view."""
    global selected_index, selected_view
    selected_index = None
    selected_view = None


# Connect event handlers to all axes
