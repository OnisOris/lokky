...
from lokky.pionmath import (
    SSolver,
)  # import the solver class from the lokky module
...
# Generate random positions and velocities (z=0)
np.random.seed(42)
...
# Form the target_matrix: displaced positions and slightly modified velocities (z=0)
...
def on_press(event):
    ...
    distances = np.hypot(
        positions[:, 0] - event.xdata, positions[:, 1] - event.ydata
    )
...
def on_motion(event):
    global selected_index, positions, state_matrix, control_signals
