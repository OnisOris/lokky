...
try:
    ...
    plt = None
    print("Warning: matplotlib is not installed. Plotting functions will be disabled.")
...
def draw_boundary_3d(ax, x_min, x_max, y_min, y_max, z_min, z_max, color="black"):
    corners = np.array([
        [x_min, y_min, z_min],
        [x_max, y_min, z_min],
        [x_max, y_max, z_min],
...
def main():
    parser = argparse.ArgumentParser(description="Enter number of simulated points")
    ...
    def update_safety_zones():
        ...
        for i in range(n_points):
            ...
            circle_top = Circle((positions[i, 0], positions[i, 1]),
                                safety_radius, facecolor=face_color,
                                edgecolor=edge_color, alpha=0.3, lw=lw)
    ...
    def draw_arrows_2d():
        ...
        for i in range(n_points):
            ...
            a = ax_top.arrow(positions[i, 0], positions[i, 1],
                             err[0], err[1],
                             color="orange", width=0.05,
                             head_width=0.3, alpha=0.6, linestyle="dashed")
            ...
            a = ax_side.arrow(positions[i, 0], positions[i, 2],
                              err[0], err[2],
                              color="orange", width=0.05,
                              head_width=0.3, alpha=0.6, linestyle="dashed")
            ...
            a = ax_front.arrow(positions[i, 1], positions[i, 2],
                               err[1], err[2],
                               color="orange", width=0.05,
                               head_width=0.3, alpha=0.6, linestyle="dashed")
    ...
    def draw_arrows_3d():
        ...
        for i in range(n_points):
            err = target_positions[i] - positions[i]
            ctrl = control_signals[i, :3]
            q = ax_3d.quiver(positions[i, 0], positions[i, 1], positions[i, 2],
                             err[0], err[1], err[2],
    ...
    def on_press(event):
        ...
        if event.inaxes == ax_top:
            ...
            for pos in positions:
                ...
                distances.append(np.hypot(proj[0] - click_screen[0],
                                          proj[1] - click_screen[1]))
            current_view = "3d"
    ...
    def on_motion(event):
        ...
        if event.inaxes == ax_top and selected_view == "top":
            ...
            if event.key is not None and ("shift" in event.key.lower() or "control" in event.key.lower()):
                ...
                new_x, new_y = get_3d_from_2d(ax_3d, event.x, event.y, positions[selected_index, 2])
